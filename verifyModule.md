...
| `lib/report.ts` + `lib/report-crypto.ts` (FR10 doctor report) | `report.test.ts` (63) | Snapshot figures (screener trend and change, per-week adherence counting only days that have arrived, Monday-start mood weeks). Window boundaries (a screening exactly at the start is kept, one a day earlier is dropped). The chart cap never under-counts self-harm endorsements. Privacy negative space: every unticked section is *absent*, not empty; medicine names never leak when unticked; notes never appear even if smuggled onto the input; no account-identifying key anywhere. PII scrub of alias and questions, Hindi preserved. `ShareRequest` rejection table (8 malformed inputs). Crypto: token uniqueness and shape, hash ≠ token, EN+HI round trip, fresh IV, wrong token fails, a `v1:` payload is rejected, single-bit tampering is detected. **Mutation-checked:** 4 deliberate bugs (leaking sections, risk under-count, future doses counted as missed, unscrubbed questions) were each caught |
...
| `open_report_share()` shipped as `SECURITY DEFINER` callable by `anon` (critical) | InsForge advisor, before deploy | Advisor rescan (0 critical); live probe with the public anon key → `permission denied` | N/A — policy fix; the function is now callable only by the server's admin role |
| Report footer (brand + URL) in the translation dictionary | `i18n.test.ts` "no untranslated English left in Hindi" | That test | Yes — it is what caught it. The guard was kept, and the string moved out of the dictionary |
| Report chart dates ~5px on phones; print margins dark; footer orphaned on a 3rd page | Live 375px render and a real PDF generated with headless Chromium | Manual (layout isn't unit-testable here); re-generate the PDF after any report or print-CSS change | N/A — caught before release |
