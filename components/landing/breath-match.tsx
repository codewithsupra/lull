...
export function BreathMatch() {
  const [phase, setPhase] = useState<{ label: string; scale: number }>({ label: "Breathe in", scale: 0.7 });

  useEffect(() => {
    let raf = 0;
    const start = performance.now();
    const loop = () => {
      const t = ((performance.now() - start) / 1000) % 11;
      const inhale = t < 5.5;
      const k = inhale ? t / 5.5 : 1 - (t - 5.5) / 5.5;
      const eased = 0.5 - Math.cos(k * Math.PI) / 2;
      setPhase({ label: inhale ? "Breathe in" : "Breathe out", scale: 0.55 + eased * 0.45 });
  ...
  return (
    <div className="relative mx-auto grid aspect-square w-full max-w-[280px] place-items-center">
      ...
      />
      <span className="relative font-mono text-xs uppercase tracking-[0.25em] text-white/90">{phase.label}</span>
    </div>
  );
}
