...
**Doctor report (FR10)**: `lib/report.ts` (pure, client-safe) builds a snapshot from the user's own data. `pickSections` *deletes* unticked sections, and check-in notes are never read. `lib/report-crypto.ts` makes **the link the key**: a 256-bit token is shown once, only SHA-256(token) is stored, and the snapshot is sealed as `r1:` AES-GCM under HKDF(token), so `HEALTH_DATA_KEY` can't open it. `anon` has no access to `report_shares`. `/r/[token]` calls `open_report_share()` (atomic live-check + view count) through the server admin client in `lib/report-server.ts`. `components/report/report-view.tsx` is hook-free and shared by the preview and the doctor page. PDF means the browser's print dialog, via the `@media print` block in `globals.css`. Re-generate a real PDF after touching either file. Creating links is Pro (`requireFeature("report")`); printing and revoking are free.
